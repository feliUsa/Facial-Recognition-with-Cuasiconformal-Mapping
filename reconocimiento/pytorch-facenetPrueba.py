...
CAM_INDEX = 0
SHOW_METRICS = True
WINDOW_NAME = "FaceMesh + FaceNet (facenet-pytorch) - Experimento en tiempo real"

# Umbral de similitud coseno para decir "misma persona"
SIM_THRESHOLD = 0.6
...
def main():
    ...
    print(f"[INFO] Cargando InceptionResnetV1 (FaceNet) en {device}...")
    resnet = InceptionResnetV1(pretrained="vggface2").eval().to(device)
    print("[INFO] Modelo FaceNet cargado.")

    # Transform para la cara
    ...
    print("[INFO] FaceMesh + FaceNet (facenet-pytorch)")
    ...
    while True:
        ...
        if main_face_info is not None:
            ...
            if current_face_bgr is not None and current_face_bgr.size > 0:
                face_rgb = cv2.cvtColor(current_face_bgr, cv2.COLOR_BGR2RGB)
                pil_img = Image.fromarray(face_rgb)

                t_rep_start = time.perf_counter()
                with torch.no_grad():
                    face_tensor = transform_face(pil_img).unsqueeze(0).to(device)
                ...
                # emb: (1, 512)
                current_embedding = emb.squeeze(0).cpu().numpy().astype(np.float32)
        ...
        if SHOW_METRICS:
            ...
            cv2.putText(frame, f"Modelo: FaceNet  Modo: {mode_str}",
                        (10, 20),
            ...
            text4 = f"T_rep_prom (FaceNet):  {avg_rep_ms:6.2f} ms"
        ...
        if not have_ref and key == ord('c') and current_embedding is not None:
            ...
            if current_face_bgr is not None:
                ...
                ref_img_path = f"ref_facemesh_facenet_{safe_name}.png"
                cv2.imwrite(ref_img_path, current_face_bgr)
                print(f"[INFO] Imagen de referencia guardada en: {ref_img_path}")

            have_ref = True
            print(f"[INFO] Embedding de referencia capturado para {ref_name} (FaceNet).")
    ...
    print(f"Tiempo rep FaceNet promedio:            {avg_rep_ms:.2f} ms")
