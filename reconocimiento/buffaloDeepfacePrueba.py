...
CAM_INDEX = 0
SHOW_METRICS = True
WINDOW_NAME = "FaceMesh + InsightFace (Buffalo_L) - Experimento en tiempo real"

# Umbral de similitud coseno (embeddings normalizados)
SIM_THRESHOLD = 0.5
...
def main():
    ...
    # ctx_id=-1 -> CPU; cambia a 0 si tienes GPU
    app.prepare(ctx_id=-1, det_size=(256, 256))
    print("[INFO] Modelo Buffalo_L cargado.")
    ...
    total_proc_time = 0.0
    total_det_time = 0.0      # FaceMesh
    total_rep_time = 0.0      # Buffalo_L (embedding)
    frames_with_rep = 0
    ...
    print("[INFO] FaceMesh + InsightFace (Buffalo_L)")
    ...
    while True:
        ...
        if main_face_info is not None:
            ...
            if current_face_bgr is not None and current_face_bgr.size > 0:
                # app.get trabaja con BGR directamente
                t_rep_start = time.perf_counter()
                faces_b = app.get(current_face_bgr)
                ...
                if len(faces_b) > 0:
                    ...
                    # Las embeddings vienen normalizadas (normed_embedding)
                    emb = faces_b[0].normed_embedding
                    current_embedding = np.array(emb, dtype=np.float32)

                    if have_ref and ref_embedding is not None:
                        # Ambos emb deben estar normalizados
                        sim_cos = compute_cosine_similarity(current_embedding, ref_embedding)
                        # Distancia L2 opcional (no estrictamente necesaria si ya está normalizado)
        ...
        if SHOW_METRICS:
            ...
            cv2.putText(frame, f"Modelo: Buffalo_L  Modo: {mode_str}",
                        (10, 20),
            ...
            text4 = f"T_rep_prom (Buffalo):  {avg_rep_ms:6.2f} ms"
            text5 = f"T_proc_prom total:     {avg_proc_ms:6.2f} ms"
        ...
        if not have_ref and key == ord('c') and current_embedding is not None:
            ...
            if current_face_bgr is not None:
                ...
                ref_img_path = f"ref_facemesh_buffalo_{safe_name}.png"
                cv2.imwrite(ref_img_path, current_face_bgr)
                print(f"[INFO] Imagen de referencia guardada en: {ref_img_path}")

            have_ref = True
            print(f"[INFO] Embedding de referencia capturado para {ref_name} (Buffalo_L).")
    ...
    print(f"Tiempo rep Buffalo_L promedio:          {avg_rep_ms:.2f} ms")
