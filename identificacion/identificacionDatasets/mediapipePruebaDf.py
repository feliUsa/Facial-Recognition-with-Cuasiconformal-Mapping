...
RESULTS_CSV = "facemesh_wider_results.csv"
...
def load_wider_annotations(root_dir: Path, split: str = "train"):
    ...
    # Determinar donde están realmente las imágenes
    ...
    while i < n:
        ...
        # Saltar luneas vacías
        if not line:
            i += 1
            continue

        # Si la lunea es la ruta de una imagen (*.jpg)
        if ".jpg" in line:
            rel_path = line
            i += 1

            # Saltar luneas vacías entre ruta y num_boxes
...
def run_facemesh_on_wider():
    ...
    # Iniciar mediapipe
    print("[INFO] Inicializando MediaPipe FaceMesh...")
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for idx, entry in enumerate(entries):
            ...
            # Construir cajas predichas a partir de landmarks
            pred_boxes = []
    ...
    print("\n========== RESULTADOS GLOBALES (MediaPipe FaceMesh + WIDER) ==========")
    ...
    print(f"Tiempo total procesando imágenes (solo FaceMesh): {total_time:.2f} s")
    print(f"Tiempo medio por imagen (solo FaceMesh): {avg_proc_time*1000:.2f} ms")
    print(f"FPS aproximado (solo FaceMesh): {fps:.2f} imágenes/s")
    print("")
