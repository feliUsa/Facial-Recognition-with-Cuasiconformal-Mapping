...
MAX_IMAGES = 1000       # None para usar todas las imágenes
RESULTS_CSV = "mtcnn_wider_results.csv"
...
def run_mtcnn_on_wider():
    ...
    # Iniciar MTCNN
    print("[INFO] Inicializando MTCNN...")
    detector = MTCNN()
    ...
    for idx, entry in enumerate(entries):
        ...
        # Construir cajas predichas a partir de MTCNN
        pred_boxes = []
        for f in faces:
            x, y, bw, bh = f.get("box", [0, 0, 0, 0])

            # MTCNN a veces devuelve coords negativas; las arreglamos
            x1 = max(0.0, float(x))
            y1 = max(0.0, float(y))
    ...
    print("\n========== RESULTADOS GLOBALES (MTCNN + WIDER) ==========")
    ...
    print(f"Tiempo total procesando imágenes (solo MTCNN): {total_time:.2f} s")
    print(f"Tiempo medio por imagen (solo MTCNN): {avg_proc_time*1000:.2f} ms")
    print(f"FPS aproximado (solo MTCNN): {fps:.2f} imágenes/s")
    print("")
